# Research Progress

Last updated: 2026-05-28T13:54:38-04:00

## Summary

| Field | Value |
| --- | --- |
| Current phase | Structured external research |
| Owner/agent | Architect, Market Research, Quant / Backtesting, Security / Compliance, UI/UX |
| Status | Completed |
| Priority | High |
| Category | Research and architecture decisions |
| Blockers | None |
| Next step | Use research summary to guide application implementation after validation/operator review |
| Validation required | Completed: source links, license notes, and CEO/CTO decisions documented |

## Research Files

| File | Status | Notes |
| --- | --- | --- |
| `docs/research/github-repos.md` | Completed | Top open-source references, license notes, and use/defer/avoid decisions |
| `docs/research/articles.md` | Completed | Official docs, articles, compliance references, and best practices |
| `docs/research/plugins-and-mcp.md` | Completed | Safe Codex, MCP, and workflow integrations |
| `docs/research/data-providers.md` | Completed | Market, options, news, earnings, fundamentals, macro, and crypto providers |
| `docs/research/backtesting-frameworks.md` | Completed | Backtesting libraries and build-vs-integrate decision |
| `docs/research/ui-patterns.md` | Completed | Financial dashboard and research-console UI patterns |
| `docs/research/agent-memory-strategy.md` | Completed | Durable project memory and agent handoff approach |
| `docs/research/security-and-compliance.md` | Completed | Security, secrets, audit, AI, and trading-risk references |
| `docs/research/recommendation-summary.md` | Completed | CEO/CTO final decisions and implementation priorities |

## Required Decision Categories

- Use now.
- Consider later.
- Avoid.
- Custom-build.
- Integrate from existing tools.
- Add to Codex/agent workflow.
- Add to the application itself.
