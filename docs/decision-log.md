...
## 2026-06-05: Plugin And Skill Adoption Policy

Decision: Govern visible Codex plugins and skills through `docs/plugin-skill-governance.md`. Browser, Superpowers, repo skills, and selected Data Analytics / visualization workflows may be used now within their scoped roles. CodeRabbit, Product Design, LSEG, Public Equity Investing, plugin creation, skill creation, and skill installation require selective use, explicit task fit, and security/risk review. Provider-like plugins must not bypass backend provider adapters, timestamp/freshness rules, citation requirements, licensing review, evidence gates, or audit logs.

Reason: Newly visible plugins can improve research, UI review, analytics, and validation, but public-market data access, third-party code review, design automation, and investment-analysis workflows introduce security, licensing, prompt-injection, and financial-risk concerns. A governed adoption policy preserves the research-first, paper-trading-first MVP boundary while letting the main Codex operator and subagents use specialized tools when they add real value.

## 2026-06-05: Paper-Only Opportunity Decision Actions
