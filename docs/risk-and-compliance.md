...
Recommendation evidence detail must be resolved from stored data, not caller-provided claims. The DB evidence resolver verifies paper-trade evidence only when the referenced row is closed, paper-only, broker-disabled, in the same ticker/instrument/strategy cohort as the recommendation, non-circular, and tied to a source recommendation with verified backtest backing in the same cohort as the paper trade. It verifies stock backtest evidence only when the referenced run is stock-only, marked `notRecommendation`, not an options proxy, `ready_for_review`, cohort-compatible, tied to the recommendation ticker through enough persisted trade rows, and backed by coherent stored reason codes, metrics, exact-set 1x/2x/3x cost sensitivity, assumptions, citations, freshness, and persisted trade counts without SQLite type coercion, non-finite JSON numeric values, duplicate or extra cost-stress rows, Julian-only timestamp parsing, timezone-naive timestamp text, malformed UTC suffix variants, or calendar-invalid UTC normalization. Database triggers apply the same safety posture before `paper_trade` recommendations or persisted paper trades can be written. Missing, open, unsafe, thin-sample, `needs_more_data`, blocked, or mismatched evidence blocks or downgrades the evidence gate.

Paper-trade eligibility requires the resolved evidence gate to be `verified` and, in the core contract, a resolver-backed evidence review whose evidence IDs exactly match the recommendation's evidence references. Every referenced evidence item must verify; a valid backtest cannot offset a missing, unsafe, or source-unverified paper-trade evidence reference. A raw `backtest_run_id` or `paper_trade_evidence_id` is only a reference and cannot make a recommendation operational by itself.
