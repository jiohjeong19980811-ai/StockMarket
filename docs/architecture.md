...
The DB package also includes a recommendation evidence resolver. It loads a stored recommendation, citations, freshness state, evidence IDs, and audit events, then resolves paper-trade evidence against persisted paper-trade rows and stock backtest evidence against persisted backtest-run rows. Paper-trade evidence is verified only when the paper trade is closed, remains paper-only, has broker execution disabled, matches the recommendation ticker, instrument, and strategy version, is not circular self-evidence, and comes from a source recommendation whose same-cohort backtest evidence still verifies. Backtest evidence is verified only when the run is stock-only, `notRecommendation`, not an options proxy, `ready_for_review`, strategy/instrument compatible, has coherent stored metrics/assumptions/citations/freshness/exact-set 1x/2x/3x cost sensitivity without SQLite JSON type coercion, non-finite JSON numeric values, duplicate or extra cost-stress rows, Julian-only timestamp parsing, timezone-naive timestamp text, malformed UTC suffix variants, or calendar-invalid UTC normalization, has a persisted trade-row count matching the run snapshot, and has enough ticker-specific persisted trades for the run's minimum sample assumption. Recommendation and paper-trade DB triggers require every referenced evidence item to verify before `paper_trade` records can be written.
