...
- API smoke can verify mock opportunity decisions are persisted through an in-memory opportunity-decision ledger and read back with audit linkage, source timestamps, freshness, reason codes, paper-only status, and no broker execution.
...
- Operator console can submit watchlist, paper-trade, avoid, and needs-more-data opportunity decisions through the paper-only ledger dry run and show audit/readback details.
...
- API smoke can verify a mock stock backtest is persisted through an in-memory ledger dry run and read back as validation-only evidence with metrics, assumptions, citations, freshness, ordered trades, `notRecommendation`, and no broker or live-trading fields.
- Recommendation evidence detail can verify stored stock backtest evidence only when the run is stock-only, `notRecommendation`, not an options proxy, `ready_for_review`, cohort-compatible, tied to the recommendation ticker, and backed by coherent stored metrics, assumptions, citations, freshness, and exact-set 1x/2x/3x cost sensitivity without SQLite type coercion, non-finite JSON numeric values, duplicate or extra cost-stress rows, Julian-only timestamp parsing, timezone-naive timestamp text, malformed UTC suffix variants, or calendar-invalid UTC normalization.
- Paper-trade eligibility requires a resolved `verified` evidence gate plus resolver-backed evidence review provenance; every referenced evidence item must verify, and raw evidence IDs or caller-set verified flags are insufficient.
- Paper-trade evidence must be non-circular and source-backed by still-verified backtest evidence; a closed paper trade cannot become evidence for its own originating recommendation.
